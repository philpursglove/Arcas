...
namespace Arcas.Server.Controllers
{
    ...
    public class SetlistController : ControllerBase
    {
        ...
        public SetlistController(IOptions<ApiKeys> options, HttpClient httpClient, HybridCache hybridCache)
        {
        ...
        public async Task<IActionResult> ArtistSearch(string searchText)
        {
            ...
            if (artistSearchResult.Artists.Any(a => a.Name.ToLowerInvariant() == searchText.ToLowerInvariant()))
            {
                ...
                var returnVal = await _hybridCache.GetOrCreateAsync($"setlist/artist/{artist.Id}", async entry =>
                        {
                            ...
                            return setlists.Where(s => s.eventDate <= DateOnly.FromDateTime(DateTime.Today)
                                                           && s.Songs.Any()).ToList();
        ...
        public async Task<IActionResult> GetSetlist(string setlistId)
        {
            ...
            var returnVal = _hybridCache.GetOrCreateAsync($"setlist/setlist/{setlistId}", async entry =>
            {
